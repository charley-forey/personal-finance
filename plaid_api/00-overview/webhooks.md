...
> **Source:** [https://plaid.com/docs/api/webhooks/](https://plaid.com/docs/api/webhooks/)
> **Section:** Overview
...
[#### Introduction to webhooks](/docs/api/webhooks/#introduction-to-webhooks)
...
[#### Configuring webhooks](/docs/api/webhooks/#configuring-webhooks)
...
[#### Webhook retries](/docs/api/webhooks/#webhook-retries)
...
If your endpoint returns a `429 Too Many Requests` response, Plaid will honor the `Retry-After` header if present, waiting the specified duration for up to a maximum of 4 hours later before the next attempt instead of applying the standard exponential backoff. The `Retry-After` value may be an integer number of seconds (e.g. `120`), an HTTP date (e.g. `Wed, 21 Oct 2026 07:28:00 GMT`), or an ISO 8601 timestamp (e.g. `2026-10-21T07:28:00Z`).

[#### Best practices for applications using webhooks](/docs/api/webhooks/#best-practices-for-applications-using-webhooks)
...
[#### Testing webhooks in Sandbox](/docs/api/webhooks/#testing-webhooks-in-sandbox)
...
[#### Example in Plaid Pattern](/docs/api/webhooks/#example-in-plaid-pattern)
